...
class TextGenerationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Возвращает prefix, target и attention mask для одного примера.
        
        Returns:
            prefix: токены префикса (75% текста)
            target: токены для предсказания (25% текста)
            attention_mask: маска внимания для target
        """
...
def create_and_save_datasets(
    ...
    Returns:
        train_loader, val_loader, test_loader, tokenizer, device
    """
    # Создаём директорию
